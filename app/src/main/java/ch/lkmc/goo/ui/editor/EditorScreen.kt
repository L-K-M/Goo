 ...
 * ViewModel (touch → resampler → stamps → GPU). Controls: a slim top rail
 * (history/reset/export) and a floating [ToolDock] whose tabs own the
 * brush palette, the levers rig, and the GOOvie strip; the dock melts
 * into a [ToolPuck] while painting so the canvas keeps the screen.
 */
@Composable
fun EditorScreen(
    onBack: () -> Unit,
...
private fun WarpEditor(
    ...
    val panel = resolvePanel(goovieMode = state.goovieMode, showLevers = showLevers)
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        Box(
                ...
                .pointerInput(bitmap) {
                    awaitEachGesture {
                        ...
                        if (stroking) {
                            ...
                            // Painting wants the whole canvas: the tray
                            // melts into its puck for the rest of the drag.
                            dockExpanded = false
                        }
                        down.consume()
            ...
            // Gated on the brush tab being the visible one: a mid-drag
            // swap strands the flag true (onValueChangeFinished never
            // fires for a slider that left composition).
            state.bitmap?.let { bmp ->
                BrushPreviewOverlay(
                    visible = adjustingBrush && dockExpanded && panel == EditorPanel.BRUSH,
    ...
    // GLSurfaceView never re-measures mid-animation, and collapsing to the
    // puck hands the whole screen back to the photo.
    androidx.compose.animation.AnimatedVisibility(
        visible = dockExpanded,
        ...
        ToolDock(
            ...
            onTabSelect = { target ->
                ...
                // underneath, and leaving the strip always lands on Brush.
...
private fun TopRail(
    ...
    Row(
        ...
        // Five beads fit every screen the app targets — no scroll. Mode
        // switching (levers/GOOvies) lives on the dock's tabs, not here.
