...
object StampBounds {
    ...
    fun of(
        ...
        val halfDown = if (profile == FalloffProfile.DRIP) {
        ...
        val y0 = (saturate((cy - halfUp) * height, roundDown = true) - margin).clampTo(height)
        val y1 = (saturate((cy + halfDown) * height, roundDown = false) + margin).clampTo(height)
